...
public class SendMapBehaviour  extends OneShotBehaviour{
	...
	private final int refresh_time = 2;// durée minimale entre deux envois de la carte à un agent (le timer incrémenté de 1 à chaque ping)
	...
	public void action() {
		if(this.agent.getExplo().getMap()!=null) {
			...
			if(this.agent.getPing()!=null) {
				...
				if(nbReceivers >0) {//si on a pas déjà envoyé récemment à tous les agents pingés, on envoie
